...
def extrai_dados_boleto(pdf_bytes):
    ...
    # ── Nome cedente ─────────────────────────────────────────────────────────
    # Coleta todos os candidatos e filtra endereços/instruções
    _PREFIXO_ENDERECO = re.compile(
        r'^(RUA|AV\b|AVENIDA|ALAMEDA|TRAVESSA|RODOVIA|ESTRADA|'
        r'REPUBLICA|NAO\s|UF\s|CEP\s|BAIRRO|CIDADE|MUNICIPIO|PC\s|PRACA)',
        re.IGNORECASE
    )
    dados['cedente_nome'] = 'NAO IDENTIFICADO'
    for _m in re.finditer(
        r'(?:Cedente|Benefici[aá]rio|Favorecido)[:\s\)]+([A-Z][A-Z\s\.&/]{5,60})',
        texto, re.IGNORECASE
    ):
        _cand = _m.group(1).strip()
        # Descarta se for endereço: prefixo suspeito, vírgula+número, instrução
        _eh_end = (
            bool(_PREFIXO_ENDERECO.match(_cand)) or
            bool(re.search(r',\s*\d|,\s*-|\d{5}-\d{3}', _cand))
        )
        if not _eh_end:
            dados['cedente_nome'] = _cand[:40]
            break
